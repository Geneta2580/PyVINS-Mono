...
class Backend:
    ...
    def window_optimize(self, active_kfs, window_imu_factors, window_landmarks,
        ...
        # [D] 零速度更新 (ZUPT)
        if is_stationary:
            zero_velocity_noise = gtsam.noiseModel.Isotropic.Sigma(3, 0.01)
            ...
            # 添加单位位姿因子
            if len(self.active_kf_gtsam_ids) > 1:
                last_kf_gtsam_id = self.active_kf_gtsam_ids[-2]
                no_motion_noise = gtsam.noiseModel.Diagonal.Sigmas(np.array([1e-4, 1e-4, 1e-4, 1e-5, 1e-5, 1e-5]))
                current_graph.add(gtsam.BetweenFactorPose3(X(last_kf_gtsam_id), X(kf_gtsam_id), gtsam.Pose3(), no_motion_noise))
